#!/usr/bin/env python3
"""
MongoDB Chat Blueprint Uploader Script

This script loops through the json_bot_docs/chat_blueprint folder and uploads JSON files
to the MongoDB database 'daemonium' collection 'chat_blueprint'. It skips template files
...
class ChatBlueprintUploader:
    """Handles uploading chat blueprint JSON files to MongoDB."""
    ...
    def _setup_logging(self) -> None:
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('chat_blueprint_upload.log'),
    ...
    def connect_to_mongodb(self) -> None:
        ...
        try:
            ...
            self.collection = self.database['chat_blueprint']
            
            self.logger.info(f"Connected to MongoDB: {host}:{port}")
            self.logger.info(f"Using database: {database}, collection: chat_blueprint")
    ...
    def _prepare_document(self, json_data: Dict[str, Any], filename: str) -> Dict[str, Any]:
        ...
        # Extract prompt blueprint data
        prompt_blueprint = json_data.get('prompt_blueprint', {})
        
        # Create a unique identifier based on author and purpose
        author = prompt_blueprint.get('author', 'unknown').replace(' ', '_').lower()
        purpose_key = prompt_blueprint.get('purpose', 'unknown')[:50].replace(' ', '_').replace(',', '').replace(':', '').lower()
        
        document = {
            '_id': f"{author}_{purpose_key}_blueprint",
            'filename': filename,
            'prompt_blueprint': {
                'purpose': prompt_blueprint.get('purpose', 'Unknown'),
                'author': prompt_blueprint.get('author', 'Unknown'),
                'meta': prompt_blueprint.get('meta', {}),
                'response_pipeline': prompt_blueprint.get('response_pipeline', []),
                'dynamic_prompt_template': prompt_blueprint.get('dynamic_prompt_template', {}),
                'conversation_rules': prompt_blueprint.get('conversation_rules', []),
                'fallback_responses': prompt_blueprint.get('fallback_responses', [])
            },
            'metadata': {
                'upload_timestamp': None,  # Will be set during upload
                'last_updated': None,      # Will be set during upload
                'source_file': filename,
                'pipeline_steps': len(prompt_blueprint.get('response_pipeline', [])),
                'resources_used': prompt_blueprint.get('meta', {}).get('resources_used', []),
                'multi_philosopher_ready': prompt_blueprint.get('meta', {}).get('multi_philosopher_ready', False)
            }
        }
        return document
    ...
    def process_chat_blueprints_folder(self, chat_blueprints_folder: str) -> Dict[str, int]:
        """Process all JSON files in the chat blueprints folder."""
        chat_blueprints_path = Path(chat_blueprints_folder)
        
        if not chat_blueprints_path.exists():
            self.logger.error(f"Chat blueprints folder not found: {chat_blueprints_path}")
            raise FileNotFoundError(f"Chat blueprints folder not found: {chat_blueprints_path}")
        ...
        json_files = list(chat_blueprints_path.glob('*.json'))
        
        if not json_files:
            self.logger.warning(f"No JSON files found in {chat_blueprints_path}")
            return stats
    ...
    def run(self, chat_blueprints_folder: str) -> None:
        """Main execution method."""
        try:
            self.logger.info("Starting chat blueprint upload process")
            
            # Connect to MongoDB
            self.connect_to_mongodb()
            
            # Process all files in the chat blueprints folder
            stats = self.process_chat_blueprints_folder(chat_blueprints_folder)
...
def main():
    ...
    chat_blueprints_folder = project_root / 'json_bot_docs' / 'chat_blueprint'
    
    try:
        # Create and run the uploader
        uploader = ChatBlueprintUploader(str(config_path))
        uploader.run(str(chat_blueprints_folder))
        
        print("Chat blueprint upload completed successfully!")
        
    except Exception as e:
        print(f"Error during chat blueprint upload: {e}")
        return 1
