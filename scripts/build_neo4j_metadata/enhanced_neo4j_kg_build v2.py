...
class EnhancedKnowledgeGraphBuilder:
    def __init__(self, config_path: str = None, 
        ...
        self.logger.info(f"  General KG Tasks: {self.general_kg_model}")
        self.logger.info(f"  Semantic Similarity: {self.semantic_similarity_model}")
        self.logger.info(f"  Concept Clustering: {self.concept_clustering_model}")
        ...
        if self.use_sentence_transformer:
            ...
            self.logger.info(f"Using Ollama for embeddings: {self.semantic_similarity_model}")
    ...
    def extract_concepts_with_ollama(self, text: str) -> List[str]:
        ...
        response = self.query_ollama(prompt, model_override=self.concept_clustering_model)
        if response:
    ...
    def analyze_relationship_with_ollama(self, content1: str, content2: str) -> str:
        """Use Ollama to analyze the relationship between two pieces of content using general KG model"""
        prompt = f"""
        ...
        response = self.query_ollama(prompt)  # Uses general_kg_model by default
        if response:
    ...
    def cluster_concepts_with_ollama(self, concepts: List[str]) -> Dict[str, List[str]]:
        """Use Ollama to cluster related philosophical concepts using concept clustering model"""
        if not concepts or len(concepts) < 2:
            return {}
            
        concepts_text = ", ".join(concepts[:20])  # Limit to prevent token overflow
        prompt = f"""
        Group these philosophical concepts into related clusters. 
        Return the result as a JSON object where keys are cluster names and values are lists of related concepts.
        Focus on philosophical relationships and thematic connections.
        ...
        if response:
            try:
                ...
                if json_start != -1 and json_end > json_start:
                    clusters = json.loads(response[json_start:json_end])
                    return clusters
            except (json.JSONDecodeError, ValueError) as e:
                self.logger.warning(f"Failed to parse concept clusters: {e}")
        
        return {}
    
    def analyze_semantic_similarity_with_embedding(self, text1: str, text2: str) -> float:
        """Analyze semantic similarity using specialized embedding model"""
        embedding1 = self.get_embedding(text1[:300])  # Limit text length
        embedding2 = self.get_embedding(text2[:300])
    ...
    def extract_philosophical_themes_with_kg_model(self, text: str) -> List[str]:
        """Extract philosophical themes using general KG model for thematic analysis"""
        prompt = f"""
        Extract the main philosophical themes from this text.
        Return only the themes as a comma-separated list, no explanations.
        Focus on major philosophical themes like ethics, metaphysics, epistemology, aesthetics, etc.
        
        Text: {text[:800]}
        
        Themes:"""
        
        response = self.query_ollama(prompt)  # Uses general_kg_model
        if response:
            themes = [theme.strip() for theme in response.split(',')]
            return [t for t in themes if t and len(t.split()) <= 3]
        return []
    
    def analyze_philosophical_influence_with_kg_model(self, philosopher1: str, philosopher2: str, context: str = "") -> str:
        """Analyze philosophical influence between two philosophers using general KG model"""
        prompt = f"""
        Analyze the philosophical influence between {philosopher1} and {philosopher2}.
        Determine the type of influence from: DIRECTLY_INFLUENCED, INDIRECTLY_INFLUENCED, CONTEMPORARY_OF, OPPOSED_TO, BUILT_UPON, CRITIQUED.
        
        Context: {context[:400] if context else 'No additional context provided'}
        
        Influence type:"""
        
        response = self.query_ollama(prompt)  # Uses general_kg_model
        if response:
            influence_types = ['DIRECTLY_INFLUENCED', 'INDIRECTLY_INFLUENCED', 'CONTEMPORARY_OF', 
                             'OPPOSED_TO', 'BUILT_UPON', 'CRITIQUED']
            for influence_type in influence_types:
                if influence_type.lower() in response.lower():
                    return influence_type
        
        return "RELATED_TO"  # Default relationship
    
    def extract_key_arguments_with_kg_model(self, text: str) -> List[str]:
        """Extract key philosophical arguments using general KG model"""
        prompt = f"""
        Extract the main philosophical arguments from this text.
        Return only the arguments as a numbered list, no explanations.
        Focus on logical arguments, premises, and conclusions.
        ...
        response = self.query_ollama(prompt)  # Uses general_kg_model
        if response:
            # Parse numbered list or comma-separated arguments
            arguments = []
            ...
            for line in lines:
                ...
                if line and (line[0].isdigit() or line.startswith('-') or line.startswith('*')):
                    ...
                    clean_arg = re.sub(r'^[\d\-\*\.\)\s]+', '', line).strip()
                    if clean_arg and len(clean_arg.split()) <= 15:  # Reasonable argument length
                        arguments.append(clean_arg)
            return arguments[:5]  # Limit to top 5 arguments
        return []
    
    def find_conceptual_similarities_with_embedding(self, concepts1: List[str], concepts2: List[str]) -> List[Tuple[str, str, float]]:
        """Find conceptual similarities between two concept lists using embedding model"""
        similarities = []
        
        for concept1 in concepts1[:10]:  # Limit to prevent explosion
            for concept2 in concepts2[:10]:
                if concept1 != concept2:  # Don't compare identical concepts
                    similarity = self.analyze_semantic_similarity_with_embedding(concept1, concept2)
                    if similarity > 0.6:  # Only include meaningful similarities
    ...
    def categorize_philosophical_content_with_clustering_model(self, content: str) -> str:
        """Categorize philosophical content using concept clustering model"""
        prompt = f"""
        Categorize this philosophical content into one of these categories:
        ETHICS, METAPHYSICS, EPISTEMOLOGY, AESTHETICS, LOGIC, POLITICAL_PHILOSOPHY, PHILOSOPHY_OF_MIND, PHILOSOPHY_OF_RELIGION, EXISTENTIALISM, PHENOMENOLOGY, ANALYTIC_PHILOSOPHY, CONTINENTAL_PHILOSOPHY
        ...
        response = self.query_ollama(prompt, model_override=self.concept_clustering_model)
        if response:
            categories = ['ETHICS', 'METAPHYSICS', 'EPISTEMOLOGY', 'AESTHETICS', 'LOGIC', 
                         'POLITICAL_PHILOSOPHY', 'PHILOSOPHY_OF_MIND', 'PHILOSOPHY_OF_RELIGION',
                         'EXISTENTIALISM', 'PHENOMENOLOGY', 'ANALYTIC_PHILOSOPHY', 'CONTINENTAL_PHILOSOPHY']
            for category in categories:
                if category.lower() in response.lower():
                    return category
        
        return "GENERAL_PHILOSOPHY"  # Default category
    ...
    def create_concept_extraction_relationships(self, all_nodes: List[Dict]):
        """Extract concepts and create concept-based relationships using specialized clustering"""
        self.logger.info("Creating concept-based relationships with enhanced clustering...")
        
        # Extract concepts from all nodes using concept clustering model
        node_concepts = {}
        all_concepts = set()
        ...
        for node_data in all_nodes:
            ...
            if content:
                # Extract concepts using specialized concept clustering model
                concepts = self.extract_concepts_with_ollama(content)
                if concepts:
                    node_concepts[node_data['node']] = concepts
                    all_concepts.update(concepts)
        
        self.logger.info(f"Extracted {len(all_concepts)} unique concepts from {len(node_concepts)} nodes")
        
        # Cluster related concepts using concept clustering model
        if len(all_concepts) > 1:
            concept_clusters = self.cluster_concepts_with_ollama(list(all_concepts))
            ...
            for cluster_name, cluster_concepts in concept_clusters.items():
                if len(cluster_concepts) > 1:  # Only create clusters with multiple concepts
                    cluster_node = Node("ConceptCluster", name=cluster_name, 
                                      concept_count=len(cluster_concepts))
                    self.graph.create(cluster_node)
                    
                    # Link concepts to their cluster
                    for concept in cluster_concepts:
                        if concept in all_concepts:  # Ensure concept exists
                            ...
                            cluster_rel = Relationship(
                                self.concept_nodes[concept], "BELONGS_TO_CLUSTER", cluster_node
                            )
                            self.graph.create(cluster_rel)
        
        # Create concept nodes and relationships to content nodes
        for node, concepts in node_concepts.items():
            for concept in concepts:
                ...
                # Create relationship between content node and concept
                rel = Relationship(node, "INVOLVES_CONCEPT", self.concept_nodes[concept])
                self.graph.create(rel)
        
        # Create enhanced relationships between nodes sharing concepts
        concept_to_nodes = {}
        for node, concepts in node_concepts.items():
            for concept in concepts:
        ...
        for concept, nodes in concept_to_nodes.items():
            for i, node1 in enumerate(nodes):
                for node2 in nodes[i+1:]:
                    # Calculate semantic similarity for shared concept relationships
                    content1 = next((n['content'] for n in all_nodes if n['node'] == node1), '')
                    content2 = next((n['content'] for n in all_nodes if n['node'] == node2), '')
                    
                    similarity_score = 0.0
                    if content1 and content2:
                        similarity_score = self.analyze_semantic_similarity_with_embedding(content1, content2)
                    
                    rel = Relationship(
                        node1, "SHARES_CONCEPT", node2,
                        shared_concept=concept,
                        semantic_similarity=similarity_score,
                        method="concept_clustering"
                    )
                    self.graph.create(rel)
                    relationships_created += 1
        
        self.logger.info(f"Created {relationships_created} concept-based relationships")
...
if __name__ == "__main__":
    ...
    CONCEPT_CLUSTERING_MODEL = args.concept_clustering_model
    
    # Legacy parameter handling for backward compatibility
    OLLAMA_MODEL = args.ollama_model  # Will override general_kg_model if provided
    EMBEDDING_MODEL = args.embedding_model  # Will override semantic_similarity_model if provided
    ...
    try:
        builder = EnhancedKnowledgeGraphBuilder(
            ...
            concept_clustering_model=CONCEPT_CLUSTERING_MODEL,
            # Legacy parameters for backward compatibility
